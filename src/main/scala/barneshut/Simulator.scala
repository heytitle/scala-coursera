...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val m = new Boundaries;
    m.minX = math.min( a.minX, b.minX  )
    m.minY = math.min( a.minY, b.minY )
    m.maxX = math.max( a.maxX, b.maxX )
    m.maxY = math.max( a.maxY, b.maxY )
    m
  }
