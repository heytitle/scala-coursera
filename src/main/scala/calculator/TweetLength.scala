...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    val remain = Var(MaxTweetLength);

    remain() = MaxTweetLength - tweetLength( tweetText() );
    remain;
  }

  def colorForRemainingCharsCount(remainigCharsCount: Signal[Int]): Signal[String] = {
      val col = Var("green");
      col()  = remainigCharsCount() match {
