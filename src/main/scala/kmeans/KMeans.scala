...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointsWithClosest = points.map( p => ( p, findClosest(p, means) ) );

    val m = pointsWithClosest.groupBy( _._2 )
      .map{ case (c, points) => ( c, points map (_._1 ) ) }

      val keys = m.keys.toList

    val nonExistingMeans = means.filterNot( keys.contains(_) )
      .map{ case p: Point => (p, () ) }
      .groupBy( _._1 )
      .map{ case (c, _ ) => ( c, Vector[Point]() ) }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
   for( point <- oldMeans )
      yield findAverage( point, classified(point) )
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    val newCenters = update( classified, means )
