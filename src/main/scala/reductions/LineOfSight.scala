...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while( i < input.length ) {
      val angle = input(i) / i;
      val maxAngle = max( output(i-1), angle );
      output(i) = maxAngle;
       i = i + 1;
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def iter( from: Int, maxAngle: Float ) : Float = {
      if( from < until && from != 0 ){
        val angle = input(from) / from;
        iter( from +1, max( maxAngle, angle ) )
      }else {
        maxAngle
      }
    }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def iter( from: Int, startingAngle: Float ) : Unit = {
      if( from < until && from != 0 ){
        val angle = input(from) / from;
        output(from) = max( startingAngle, angle );
        iter( from +1, output(from) )
      }
    }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = tree match {
      case Leaf( f, u, previousAngle ) =>  {
        downsweepSequential( input, output, max( previousAngle, startingAngle ), f, u ) 
      }
      case Node(l,r) => { 
