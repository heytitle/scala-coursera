...
import FloatOps._
  ...
  test("Body.updated should take bodies in a Fork into account") {
    val b1 = new Body(123f, 18f, 26f, 0f, 0f)
    val b2 = new Body(524.5f, 24.5f, 25.5f, 0f, 0f)
    val b3 = new Body(245f, 22.4f, 41f, 0f, 0f)
  ...
  test("'Combine") {
    ...
    val sm = new SectorMatrix(boundaries, SECTOR_PRECISION)
    sm += body
    val ss = new SectorMatrix(boundaries, SECTOR_PRECISION)
    ss += body

    val com = sm.combine(ss)

    val size = com(2, 3).size;
    assert( size == 2, s",correct size is $size")
  }


  // Simul
  // test("update boundaries"){
  //   val b = new Boundaries;
  //   val body = new Body(5, 25, 47, 0.1f, 0.1f)
  //   val s  = new Simulator( taskSupport, 2 );

  //   assert( s.updateBoundaries(b, body) );
