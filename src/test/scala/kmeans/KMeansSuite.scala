...
class KMeansSuite extends FunSuite {
  ...
  test("'update centers'"){
    ...
    val p2 = new Point(2,4,8)
    val points: GenSeq[Point] = IndexedSeq(p1,p2)
    val classified = GenMap(( p1, points ) )
    val newCenters = update( classified, IndexedSeq(p1) )
    val expected: GenSeq[Point] = IndexedSeq(new Point(1,2,4));
    assert( newCenters.corresponds(expected){ (p1,p2) => p1 equals p2 })
  }
  ...
  test("'converged'"){
    ...
    val points: GenSeq[Point] = IndexedSeq(p1,p2)
    val np1 = new Point(1,1,1);
    val np2 = new Point(3,3,3)
    val newPoints: GenSeq[Point] = IndexedSeq( np1, np2 )
    val con = converged(15)( points, newPoints );
    assert( con == true )
  }
  ...
  test("'not converged'"){
    ...
    val points: GenSeq[Point] = IndexedSeq(p1,p2)
    val np1 = new Point(1,1,1);
    val np2 = new Point(1,1,3)
    val newPoints: GenSeq[Point] = IndexedSeq( np1, np2 )
    val con = converged(3)( points, newPoints );
    assert( con == false )
  }
